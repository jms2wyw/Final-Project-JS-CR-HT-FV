...
public class GameController {
    ...
    @ResponseStatus(HttpStatus.OK)
    public List<GameViewModel> getGamesByEsrbRating(@PathVariable String rating) {
        return gameServiceLayer.findGamesByEsrbRating(rating);
    }

    @GetMapping("/games/titles/{title}")
    @ResponseStatus(HttpStatus.OK)
    public GameViewModel getGameByTitle(@PathVariable String title) {
    ...
    @PutMapping("/games")
    @ResponseStatus(HttpStatus.OK)
    public GameViewModel updateGame(@RequestBody GameViewModel gameViewModel) {
